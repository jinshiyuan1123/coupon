...
class TcgService
{
    ...
    public function gameList($product_type, $game_type = 'RNG', $client_type= 'web', $platform = 'html5', $page = 1, $page_size = 1000)
    {
        set_time_limit(0);
        $getParams = array('product_type' => $product_type, 'method' => 'tgl' , 'platform' => $platform, 'game_type' => $game_type, 'client_type' => $client_type, 'page' => $page, 'page_size' => $page_size);
        $json_request = json_encode($getParams);
        $params =  $this->encryptText(json_encode($getParams),$this->desKey);
        //SIGN
        $sign = hash('sha256', $params . $this->signKey);
