...
class AllbetService{
    ...
    public function login($username,$password){
        ...
        $post_data = array('apiAccount'=>$this->comId,'userName'=>$username,'password'=>$password,'language'=>$lang,'code'=>$code);
    ...
    * 游戏记录
    */
    public function betrecord($startTime,$endTime)
    {
        $code = $this->salt.md5($this->comKey.$this->comId.$startTime.$endTime.$this->salt);
        $url = "http://".$this->domain."/api/allbet/betlogpieceofhistories.ashx";
        $post_data = array('apiAccount'=>$this->comId,'startTime'=>$startTime,'endTime'=>$endTime,'code'=>$code);
        $receive = $this->send_post($url,$post_data);
        return $receive;
    }
