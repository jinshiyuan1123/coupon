...
class MemberDailiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach($data as $v){
			$Couponsbusiness=Couponsbusiness::Where('businessid',$v->id)->pluck('id')->toArray();
			$v->coupons=count($Couponsbusiness);
			$usecoupons=Coupons::Where('businessid',$v->id)->Where('status',2)->pluck('id')->toArray();
			$v->usecoupons=count($usecoupons);
			$paymoney=PayRecord::Where('businessid',$v->id)->Where('status',1);
			$v->paymoney=$paymoney->sum('money');
		}
