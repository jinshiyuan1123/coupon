...
<!---->
	...
	<div class="container">
		...
		<div class="header_pic">
			<div><img src="{{$data->headpic}}" /></div>
		</div>
		<div class="right_con">
			<div class="demo">
            	<!--用户名and发布时间-->
            	<div class="use">
                	<div class="usename"><span>{{$data->nichen}}</span><em class="pub-time">{{$data->created_at}}</em></div>
                </div>
                <!--分享的内容-->
                <p class="fx_content">{{$data->content}}</p>
                <!--分享的图片-->
                <div class="my-gallery">
				@if($data->imgurltotal > 0)
				  @foreach($data->imgurl as $c)
                    <figure>
                ...
                <!--显示的位置-->
               @if($data->uid==$_member->id)<div class="fx_address"><form action="{{ route('wap.delectmymoments',['id'=>$data->id]) }}" method="post" name="form1"><input type="button" value="删除" class="ajax-submit-btn"></form></div>@endif <div class="fx_address_footbook">({{$data->message}})</div><div id="{{$data->id}}" class="@if(in_array($data->id,$arrayzan)) fx_address_zan2 @else fx_address_zan @endif" onclick="zan(this.id)">(<span class="{{$data->id}}">{{$data->zan}}</span>)</div><div class="fx_address">{{$data->MomentsClassname}}</div>
            </div>
		</div>
	</div>	
...
<!---->	
                <div class="userInfo setCard" style="width:95%;margin:0 auto">
                    <form action="{{ route('wap.postmymomentsc')}}" method="post" name="form1">
                        <dl class="set_card">
                            <dd>
                            ...
                            <dd>
							    <input type="hidden" name="momentsid" value="{{$data->id}}">
