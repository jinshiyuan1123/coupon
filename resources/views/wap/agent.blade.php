...
$(function(){
	...
	<style type="text/css">
		.normal{
		  margin:10px auto;
		  background-image: url({{ $_member->Businessheadpic }});
		...
		.outmoney{
			...
			border: 1px solid #e0dfdf;
                ...
                <a href="{{ route('wap.set') }}" style="color:#afafaf"><div class="set">设置 </div>	</a>
                <a href="{{ route('wap.Businessupload') }}" class="c_blue"><div class="normal"></div></a>
				<div class="nichen">
                                    {{ $_member->Businessname }}
				</div>	
				<div class="nichen" style="margin-top:10px;">
                      今日收入：{{$todaymoney}}
				</div>					
				<a href="{{route('wap.draw')}}"><div class="nichen" style="margin-top:10px;">
                      余额：{{ $_member->money }}
				</div>	</a>
<!--商家管理菜单-->
<div class="Business_nav">
<ul><a href="{{ route('wap.card') }}"><li>优惠券</li></a><a href="{{route('wap.member_offline_recharge')}}"><li>账单</li></a><a href="{{route('wap.ad')}}"><li>广告</li></a></ul>
</div>
@else
